...
#         continue
    ...
    for consider_cat in D.modification_counter.keys():
        ...
        for consider_col in consider_cols:
            ...
            renamed_columns = {'%s-%s' % (consider_col, ccat): ccat.replace('/', '_') for ccat in D.modification_counter.keys()}
            ...
            consider = consider[['sae', 'sae_highlight', 'aave'] + list(set(renamed_columns.values()).difference({'sae', 'sae_highlight', 'aave'}))].copy()
            consider['dataset'] = bn
